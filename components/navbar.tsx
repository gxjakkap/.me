...
export default function Navbar() {
  ...
  const Links = ({
    ...
    cur: string | null
    menuRef: MutableRefObject<HTMLInputElement | null>
  }) => {
    const closeMenu = () => {
      ;(menuRef as MutableRefObject<HTMLInputElement>).current.checked = false
    }
    const linkClickAction = (path: CurState) => {
      closeMenu()
      setCur(path)
    }
    return (
      <>
        <li>
          <Link
            className={`text-[#313638] dark:text-[#e0e0e0] ${cur === "home" ? "underline decoration-1 underline-offset-4" : ""}`}
            href="/#main"
            onClick={() => linkClickAction("home")}
          >
            Home
          </Link>
        </li>
        <li>
          <Link
            className={`text-[#313638] dark:text-[#e0e0e0] ${cur === "more" ? "underline decoration-1 underline-offset-4" : ""}`}
            href="/#more"
            onClick={() => linkClickAction("more")}
          >
            About
          </Link>
        </li>
        <li>
          <Link
            className={`text-[#313638] dark:text-[#e0e0e0] ${cur === "edu" ? "underline decoration-1 underline-offset-4" : ""}`}
            href="/#education"
            onClick={() => linkClickAction("edu")}
          >
            Education
          </Link>
        </li>
        <li>
          <Link
            className={`text-[#313638] dark:text-[#e0e0e0] ${cur === "skills" ? "underline decoration-1 underline-offset-4" : ""}`}
            href="/#skills"
            onClick={() => linkClickAction("skills")}
          >
            Skills
          </Link>
        </li>
        <li>
          <Link
            className={`text-[#313638] dark:text-[#e0e0e0] ${cur === "proj" ? "underline decoration-1 underline-offset-4" : ""}`}
            href="/project"
            onClick={() => linkClickAction("proj")}
          >
            Projects
          </Link>
        </li>
        <li>
          <Link
            className={`text-[#313638] dark:text-[#e0e0e0] ${cur === "blog" ? "underline decoration-1 underline-offset-4" : ""}`}
            href="/blog"
            onClick={() => linkClickAction("blog")}
          >
            Blog
          </Link>
        </li>
        <li>
          <Link
            className={`text-[#313638] dark:text-[#e0e0e0] ${cur === "contact" ? "underline decoration-1 underline-offset-4" : ""}`}
            href="/contact"
            onClick={() => linkClickAction("contact")}
          >
            Contact
          </Link>
        </li>
          ...
          <div className="fixed right-0 top-0 z-40 size-full translate-x-full overflow-y-auto overscroll-y-none transition duration-500 peer-checked:translate-x-0">
            <div className="float-right min-h-full w-[85%] bg-[#f2f2f2] px-6 pt-12 font-inter shadow-2xl dark:bg-[#1a1a1a]">
              ...
              <Link
                ...
                onClick={() => {
                  ;(mobileMenuRef as any).current.checked = false
                }}
              >
                Heavy Rotation
              </Link>
        ...
        <div className="ml-auto mr-[10%] flex">
          <ul className="flex list-none gap-x-5 font-inter text-base lg:text-[1rem]">
            ...
            <div className="group relative break-keep font-inter">
              <p className="flex gap-x-1 text-[#313638] dark:text-[#e0e0e0]">
                ETC <ChevronDownIcon className="w-4 text-[#313638] dark:text-[#e0e0e0]" />
              </p>
              <div className="absolute z-10 hidden p-2 group-hover:flex">
                <ul>
                  <li>
                    <Link href="/etc/heavyrotation">
                      <div className="flex gap-x-1 text-[#313638] dark:text-[#e0e0e0]">
                        <span>Heavy</span>
                        <span>Rotation</span>
                      </div>
                    </Link>
