...
const dataMap: { name: SkillCategory; displayName: string }[] = [
  { name: "front", displayName: "Frontend" },
  { name: "back", displayName: "Backend" },
  { name: "tools", displayName: "Tools / Other languages" },
]
    ...
    >
      ...
      <div className="mt-10 flex flex-col gap-y-8">
        {dataMap.map((cur) => (
          <div
            key={cur.name}
            className="flex max-w-screen-sm flex-col gap-y-4 md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl 2xl:max-w-screen-2xl"
          >
            ...
            <div className="mx-auto flex flex-wrap gap-x-5 gap-y-8 lg:mx-0">
              {stack[cur.name]?.map((ea) => (
                <div
                  key={ea.name}
                  className="flex size-20 flex-col gap-y-2 rounded-2xl lg:gap-y-1"
                  suppressHydrationWarning
                >
                  <StackImageWithDark
                    className="mx-auto size-14"
                    src={ea.image}
                    srcDark={ea.image_dark || null}
                    alt={ea.name}
                  />
