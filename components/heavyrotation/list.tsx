...
export const HeavyRotationList = () => {
  const { data, error, isLoading } = useSWR("toptracks", guntxFetching)
  ...
  if (isLoading) return <HeavyRotationLoading />
    ...
    return (
      <p className="text-center font-inter lg:text-left">
        Error loading Jakka&apos;s heavy rotation. Please try again.
      </p>
    )
  else {
    return (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="relative aspect-square w-full max-w-md overflow-hidden rounded-lg bg-black text-white">
              <div
                ...
                style={{
                  backgroundImage: `url('${data.items[selectedTrack ?? 0].image_link || "https://placehold.co/400"}')`,
                }}
              ></div>
              <div className="absolute inset-0 bg-black/60"></div>
              <div className="relative z-10 flex h-full flex-col p-6">
                <div className="mb-4 flex items-center justify-between">
                  <h2 className="font-inter text-xl font-bold text-[#e0e0e0] dark:text-[#e0e0e0]">
                    {data.items[selectedTrack ?? 0].name}
                  </h2>
                  <button
                    type="button"
                    onClick={closeModal}
                ...
                <p className="text-[#e0e0e0] dark:text-[#e0e0e0]">
                  by {data.items[selectedTrack ?? 0].artist}
                </p>
                <div className="grow"></div>
                <div className="pb-2 lg:pb-10">
                  <iframe
                    title={`${data.items[selectedTrack ?? 0].name}`}
