...
export default function Contact() {
  ...
  const [formStatus, setFormStatus] = useState<
    "success" | "failed" | "pending" | "loading" | "captcha"
  >("pending")

  return (
    <main className="mx-auto flex w-3/4 grow flex-col pt-4 lg:ml-[25%] lg:mr-0 lg:mt-14 lg:w-1/2 lg:pt-10">
      <h1 className="text-center font-inter text-4xl font-medium lg:text-left lg:text-3xl">
        Contact me📥
      </h1>
      <div className="mt-4 flex flex-col">
        <div className="mt-3 mb-3 flex flex-col lg:flex-row gap-y-2 lg:gap-x-8">
          <a
            className="flex gap-x-3 font-inter"
            href="https://discordapp.com/users/480228633525354506"
            target="_blank"
          ...
          <a
            className="flex gap-x-3 font-inter"
            href="https://instagram.com/guntxjakka"
            target="_blank"
            rel="noopener noopener,noreferrer"
          >
          ...
          <a
            className="flex gap-x-3 font-inter"
            href="https://www.linkedin.com/in/jchalermphanaphan/"
            target="_blank"
            rel="noopener noopener,noreferrer"
          >
          ...
          <a
            className="flex gap-x-3 font-inter"
            href="mailto:gunt@guntxjakka.me"
            target="_blank"
            rel="noopener noopener,noreferrer"
          >
        ...
        <div className="mb-3">
          <ContactForm
            sa={handleFormSubmit}
            stts={setTurnstileStatus}
            sfs={setFormStatus}
            tts={turnstileStatus}
          />
          {formStatus === "success" && <ContactSuccessModal setFormStatus={setFormStatus} />}
          {formStatus === "failed" && (
            <ContactErrorModal setFormStatus={setFormStatus} type="failed" />
          )}
          {formStatus === "captcha" && (
            <ContactErrorModal setFormStatus={setFormStatus} type="captcha" />
          )}
        </div>
      </div>
    </main>
