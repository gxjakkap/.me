...
export function ContactForm({ sa, stts }: { sa: (formData: FormData) => Promise<void>, stts: Dispatch<SetStateAction<"success" | "error" | "expired" | "required">> }) {
    return (
        <form className="flex flex-col gap-y-3" action={sa}>
            <div className="">
                <label className="font-inter" htmlFor="name">Your name</label>
                <input className="w-full px-4 py-[0.65rem] box-border border-2 border-solid border-[#ccc] rounded-[0.4rem] bg-[#f8f8f8] font-inter resize-none" type="text" name="name" id="name" placeholder="Rejoice"/>
            </div>

            <div className="">
                <label className="font-inter" htmlFor="email">Email</label>
                <input type="email" name="email" id="email" placeholder="rejoice@kmutt.ac.th" className="w-full px-4 py-[0.65rem]    box-border border-2 border-solid border-[#ccc] rounded-[0.4rem] bg-[#f8f8f8] font-inter resize-none" />
            </div>
