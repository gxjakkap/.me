...
function Heading2({ children, ...props }) {
  ...
  return (
    <h2 id={headingHash} {...props}>
      <a className="hover:underline" href={`#${headingHash}`}>
        {children}
      </a>
    </h2>
  )
...
class Hpl extends Component {
  render() {
    ...
    return (
      <a style={style} href={this.props.href} target="_blank" rel="noopener noreferrer">
        {this.props.children}
      </a>
    )
  }
  ...
  return (
    <LazyLoadImage
      style={{
        ...
        maxHeight: "550px",
        marginLeft: "auto",
        marginRight: "auto" /* , borderRadius: '0.5rem' */,
      }}
      src={`https://${file.url}`}
      alt={title}
    />
